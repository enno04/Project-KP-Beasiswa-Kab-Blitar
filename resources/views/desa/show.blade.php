    ...
    <div class="space-y-6 pb-20">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                ...
                @if(!$semuaDokumenValid && !$pendaftaran->rekomendasiDesa)
                <div class="card">
                    ...
                    <div class="card-body space-y-4">
                        ...
                        @if($pendaftaran->rekomendasiDesa)
                            @if($pendaftaran->rekomendasiDesa->status_kecamatan === 'disetujui')
                                <div class="alert alert-success"><i data-lucide="check-circle" class="w-5 h-5"></i><div><strong>Disetujui oleh Kecamatan</strong><p class="text-xs mt-1">Diunggah: {{ $pendaftaran->rekomendasiDesa->tanggal_rekomendasi?->format('d M Y H:i') }}</p></div></div>
                            ...
                            @else
                                <div class="alert alert-warning"><i data-lucide="clock" class="w-5 h-5"></i><div><strong>Menunggu Verifikasi Kecamatan</strong><p class="text-xs mt-1">Diunggah: {{ $pendaftaran->rekomendasiDesa->tanggal_rekomendasi?->format('d M Y H:i') }}</p></div></div>
                            @endif
                        ...
                        {{-- Upload Form --}}
                        @if(!$pendaftaran->rekomendasiDesa || $pendaftaran->rekomendasiDesa->status_kecamatan === 'ditolak')
                        <form action="{{ route('desa.rekomendasi', $pendaftaran->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4 pt-4 border-t border-slate-100">
                            ...
                            <div class="flex justify-end">
                                <button type="submit" class="btn btn-primary"><i data-lucide="send" class="w-4 h-4"></i> Kirim ke Kecamatan</button>
                            </div>
                        </form>
                        @endif
                    </div>
