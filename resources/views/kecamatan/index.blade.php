        ...
        <x-slot:actions>
            <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                ...
                <div x-show="open" x-transition style="display: none;" class="absolute left-0 sm:right-0 sm:left-auto top-full mt-2 w-72 bg-white border border-slate-200 rounded-lg shadow-lg z-50 overflow-hidden">
                    <a href="{{ route('kecamatan.program.export', ['programSlug' => $program->slug, 'jalurSlug' => $jalur?->slug] + request()->except('status') + ['export_type' => 'disetujui_keduanya']) }}" class="flex items-start px-4 py-3 text-sm font-medium text-slate-700 hover:bg-emerald-50 hover:text-emerald-700 border-b border-slate-100 transition-colors">
                        <i data-lucide="check-square" class="w-4 h-4 mr-2.5 shrink-0 text-emerald-600 mt-0.5"></i>
                        <span>Disetujui Keduanya (Kec & DPMD)</span>
                    </a>
                    <a href="{{ route('kecamatan.program.export', ['programSlug' => $program->slug, 'jalurSlug' => $jalur?->slug] + request()->except('status') + ['export_type' => 'disetujui_kecamatan']) }}" class="flex items-start px-4 py-3 text-sm font-medium text-slate-700 hover:bg-blue-50 hover:text-blue-700 border-b border-slate-100 transition-colors">
                        <i data-lucide="check" class="w-4 h-4 mr-2.5 shrink-0 text-blue-500 mt-0.5"></i>
                        <span>Disetujui Kecamatan</span>
                    </a>
                    <a href="{{ route('kecamatan.program.export', ['programSlug' => $program->slug, 'jalurSlug' => $jalur?->slug] + request()->except('status') + ['export_type' => 'disetujui_dpmd']) }}" class="flex items-start px-4 py-3 text-sm font-medium text-slate-700 hover:bg-purple-50 hover:text-purple-700 border-b border-slate-100 transition-colors">
                        <i data-lucide="check-circle" class="w-4 h-4 mr-2.5 shrink-0 text-purple-600 mt-0.5"></i>
                        <span>Disetujui DPMD</span>
                    </a>
                    <a href="{{ route('kecamatan.program.export', ['programSlug' => $program->slug, 'jalurSlug' => $jalur?->slug] + request()->except('status') + ['export_type' => 'proses_kecamatan']) }}" class="flex items-start px-4 py-3 text-sm font-medium text-slate-700 hover:bg-amber-50 hover:text-amber-700 border-b border-slate-100 transition-colors">
                        <i data-lucide="clock" class="w-4 h-4 mr-2.5 shrink-0 text-amber-500 mt-0.5"></i>
                        <span>Proses Persetujuan Kecamatan</span>
                    </a>
                    <a href="{{ route('kecamatan.program.export', ['programSlug' => $program->slug, 'jalurSlug' => $jalur?->slug] + request()->except('status') + ['export_type' => 'proses_dpmd']) }}" class="flex items-start px-4 py-3 text-sm font-medium text-slate-700 hover:bg-orange-50 hover:text-orange-700 border-b border-slate-100 transition-colors">
                        <i data-lucide="hourglass" class="w-4 h-4 mr-2.5 shrink-0 text-orange-500 mt-0.5"></i>
                        <span>Proses Persetujuan DPMD</span>
                    </a>
                    <a href="{{ route('kecamatan.program.export', ['programSlug' => $program->slug, 'jalurSlug' => $jalur?->slug] + request()->query()) }}" class="flex items-start px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition-colors">
